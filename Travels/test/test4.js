/**window.addEventListener('load', function () {
    ...
    if (document.getElementById('accountName')) {
        ...
        xmlhttp.open('GET', 'PHP/getMyPhotos.php', true);
        xmlhttp.send();
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            if (xmlhttp.responseText == 'nofavorite')
                imgDiv.innerHTML = '<h3 class="nosignin">You own no photos.</h3>';
    ...
    function loadImg() {
        ...
        if (imgArray.length == 0)
            imgDiv.innerHTML = '<h3 class="nosignin" style="text-align: center"><a href="upload.php">Upload</a> to show more.</h3>';
        else {
            let pageNum;
        ...
        function deleteMyPhoto() {
            ...
            let parentRow = eventBT.parentNode.parentNode.parentNode.parentNode;
            let phpurl = 'PHP/delete.php?deleteType=deleteMyPhoto&imageID=' + imageID;
        ...
        function outputImgFullRow() {
            ...
            imgFullRow += '<div class="title-div"><a href="imageDetail.php?imageID=' + imgArray[currentImg].imageID + '">';
            imgFullRow += '<div><h3>' + imgArray[currentImg].title + '</h3></div></a></div>';
            imgFullRow += '<div><p><a class="btn btn-lg btn-primary" href="modify.php?imageID='
                + imgArray[currentImg].imageID + '">Modify</a>';
            imgFullRow += '<a class="btn btn-lg btn-danger delete-btn" title="' + imgArray[currentImg].imageID
                + '">Delete</a></p></div></div></div>';
            currentImg++;
            return imgFullRow;
        }
